    ...
    persist(
      (set, get) => ({
        ...
        convertToEMI: (templateId) => {
          ...
          // Delete the old recurring template (this won't affect transactions since we've already updated them)
          get().deleteTemplate(templateId);

          // Clean up: Remove any auto-generated transactions that might have been created
          // (in case createEMI was called, though we're now bypassing it)
          const autoGeneratedTransactions = useExpenseTransactionsStore.getState().transactions.filter(
            (t) => t.emiId === newEMI.id && 
                   t.recurringTemplateId === undefined &&
