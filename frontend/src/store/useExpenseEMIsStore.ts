    ...
    persist(
      (set, get) => ({
          ...
          activeEMIs.forEach((emi) => {
            ...
            if (deductionDate <= today && emi.completedInstallments < emi.totalInstallments) {
              ...
              if (existingTransactions.length === 0) {
                ...
                const updateData: Partial<ExpenseEMI> = {
        ...
        convertToRecurring: (emiId) => {
          ...
          const originalTransactions = transactionsStore.transactions.filter((t) => t.emiId === emiId);
          ...
          // Verify transactions were updated before deleting EMI
          const remainingTransactions = transactionsStore.transactions.filter((t) => t.emiId === emiId);
          if (remainingTransactions.length > 0) {
            // Rollback: remove the template we just created
            useRecurringExpensesStore.setState((state) => ({
              templates: state.templates.filter(t => t.id !== newTemplate.id),
            }));
            throw new Error(`Failed to update ${remainingTransactions.length} transaction(s) during conversion`);
          }

          // Delete the old EMI (transactions have been updated, so this should succeed)
          get().deleteEMI(emiId);

          // Clean up: Remove any auto-generated transactions that might have been created
          // These would be transactions that reference the new template but were NOT in the original transactions
          const allNewTemplateTransactions = transactionsStore.transactions.filter(
            (t) => t.recurringTemplateId === newTemplate.id && t.emiId === undefined
          );
          const autoGeneratedTransactions = allNewTemplateTransactions.filter(
