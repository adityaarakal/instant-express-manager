 ...
 * 
 * Note: PDF export functions are lazy-loaded from transactionExportPDF.ts
 * to reduce initial bundle size. jspdf and jspdf-autotable are only loaded
 * when PDF export is actually used.
 */
...
/**
 * Lazy-loaded PDF export functions
 * These functions dynamically import the PDF module only when needed
 */

export const exportIncomeTransactionsToPDF = async (
  transactions: IncomeTransaction[],
  accounts: BankAccount[],
  filename?: string,
): Promise<void> => {
  const { exportIncomeTransactionsToPDF: exportPDF } = await import('./transactionExportPDF');
  return exportPDF(transactions, accounts, filename);
};

export const exportExpenseTransactionsToPDF = async (
  transactions: ExpenseTransaction[],
  accounts: BankAccount[],
  filename?: string,
): Promise<void> => {
  const { exportExpenseTransactionsToPDF: exportPDF } = await import('./transactionExportPDF');
  return exportPDF(transactions, accounts, filename);
};
...
export const exportSavingsTransactionsToPDF = async (
  ...
  const { exportSavingsTransactionsToPDF: exportPDF } = await import('./transactionExportPDF');
  return exportPDF(transactions, accounts, filename);
};

export const exportTransferTransactionsToPDF = async (
  transfers: TransferTransaction[],
  accounts: BankAccount[],
  filename?: string,
): Promise<void> => {
  const { exportTransferTransactionsToPDF: exportPDF } = await import('./transactionExportPDF');
  return exportPDF(transfers, accounts, filename);
};
